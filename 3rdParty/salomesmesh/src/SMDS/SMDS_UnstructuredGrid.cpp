                                        ...
                                        std::vector<int>& idCellsOldToNew, int newCellSize)
{
  ...
  //if (this->FaceLocations)								// NATAES Son - 12.2024
  vtkIdTypeArray* thisFaceLocations = GetFaceLocations();	// NATAES Son - 12.2024
  vtkIdTypeArray* thisFaces = GetFaces();					// NATAES Son - 12.2024
  if (thisFaceLocations)									// NATAES Son - 12.2024
    {
      ...
      newFaces->Initialize();
      //newFaces->Allocate(this->Faces->GetSize());	// NATAES Son - 12.2024
	  newFaces->Allocate(thisFaces->GetSize());		// NATAES Son - 12.2024
          ...
          if (newTypes->GetValue(newCellId) == VTK_POLYHEDRON)
            {
               ...
               //int oldFaceLoc = this->FaceLocations->GetValue(i);		// NATAES Son - 12.2024
               //int nCellFaces = this->Faces->GetValue(oldFaceLoc++);	// NATAES Son - 12.2024
			   int oldFaceLoc = thisFaceLocations->GetValue(i);			// NATAES Son - 12.2024
               int nCellFaces = thisFaces->GetValue(oldFaceLoc++);		// NATAES Son - 12.2024
               newFaces->InsertNextValue(nCellFaces);
               for (int n=0; n<nCellFaces; n++)
                 {
                   //int nptsInFace = this->Faces->GetValue(oldFaceLoc++);	// NATAES Son - 12.2024
				   int nptsInFace = thisFaces->GetValue(oldFaceLoc++);		// NATAES Son - 12.2024
                   newFaces->InsertNextValue(nptsInFace);
                   for (int k=0; k<nptsInFace; k++)
                     {
                       //int oldpt = this->Faces->GetValue(oldFaceLoc++);	// NATAES Son - 12.2024
					   int oldpt = thisFaces->GetValue(oldFaceLoc++);		// NATAES Son - 12.2024
      ...
      newFaces->Squeeze();
      //this->SetCells(newTypes, newLocations, newConnectivity, newFaceLocations, newFaces);	// NATAES Son - 12.2024
	  this->SetCells(newTypes, newLocations, newConnectivity, thisFaceLocations, thisFaces);	// NATAES Son - 12.2024
      newFaceLocations->Delete();
      newFaces->Delete();
    }
  else
  {
    //this->SetCells(newTypes, newLocations, newConnectivity, FaceLocations, Faces);		// NATAES Son - 12.2024
	this->SetCells(newTypes, newLocations, newConnectivity, thisFaceLocations, thisFaces);	// NATAES Son - 12.2024
  }
...
//FIXME: vtk9
  //#if VTK_VERSION_NUMBER < VTK_VERSION_CHECK(9,3,0)	// NATAES Son - 12.2024
  #if VTK_MAJOR_VERSION < 9								// NATAES Son - 12.2024
