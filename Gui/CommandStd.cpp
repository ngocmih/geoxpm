...
// NATAES Ngoc - 1.2025 - start - doi thanh thong tin cua GeoXPM
//===========================================================================
//  Std_About
//===========================================================================
...
Action* StdCmdAbout::createAction()
{
    ...
    pcAction->setShortcut(QString::fromLatin1(getAccel()));
    // Needs to have AboutRole set to avoid duplicates if adding the about action more than once on
    // macOS
...
void StdCmdAbout::activated(int iMsg)
{
    ...
    //     Gui::Dialog::AboutDialogFactory::defaultFactory(); boost::scoped_ptr<QDialog>
    //     dlg(f->create(getMainWindow())); dlg->exec();
}
...
void StdCmdRestartInSafeMode::activated(int iMsg)
{
    ...
    restartBox.setText(
        QObject::tr("Are you sure you want to restart FreeCAD and enter safe mode?"));
    restartBox.setInformativeText(
        QObject::tr("Safe mode temporarily disables your configuration and addons."));
...
void StdCmdDlgPreferences::activated(int iMsg)
{
    ...
    ParameterGrp::handle hGrp =
        App::GetApplication().GetParameterGroupByPath("User parameter:BaseApp/Preferences");
    ...
    : Command("Std_FreeCADDonation")
{
    ...
    sMenuText = QT_TR_NOOP("Support ..");  // NATAES Ngoc - 1.2025
    ...
    : Command("Std_FreeCADWebsite")
{
    ...
    sMenuText = QT_TR_NOOP(".. W&ebsite");  // NATAES Ngoc - 1.2025
...
void StdCmdFreeCADPowerUserHub::activated(int iMsg)
{
    Q_UNUSED(iMsg);
    std::string defaulturl =
        QCoreApplication::translate(this->className(), "https://wiki.freecad.org/Power_users_hub")
            .toStdString();
    ParameterGrp::handle hURLGrp = App::GetApplication().GetParameterGroupByPath(
        "User parameter:BaseApp/Preferences/Websites");
    ...
    : Command("Std_FreeCADForum")
{
    ...
    sMenuText = QT_TR_NOOP(".. &Forum");  // NATAES Ngoc - 1.2025
    ...
    : Command("Std_FreeCADFAQ")
{
    ...
    sMenuText = QT_TR_NOOP(".. FA&Q");  // NATAES Ngoc - 1.2025
...
void StdCmdFreeCADFAQ::activated(int iMsg)
{
    Q_UNUSED(iMsg);
    std::string defaulturl =
        QCoreApplication::translate(this->className(),
...
Gui::Action* StdCmdUserEditMode::createAction()
{
    ...
    for (auto const& uem : Gui::Application::Instance->listUserEditModes()) {
        ...
        act->setIcon(BitmapFactory().iconFromTheme(
            qPrintable(QString::fromLatin1("Std_UserEditMode") + modeName)));
