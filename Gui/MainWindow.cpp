    ...
    : QMainWindow(parent, f /*WDestructiveClose*/)
{
    ...
    connect(&d->saveStateTimer, &QTimer::timeout, [this]() {
    ...
    connect(d->windowMapper,
            qOverload<QWidget*>(&QSignalMapper::mapped),
            this,
            &MainWindow::onSetActiveSubWindow);
#elif QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
    connect(d->windowMapper, &QSignalMapper::mappedWidget, this, &MainWindow::onSetActiveSubWindow);
#else
    connect(d->windowMapper, &QSignalMapper::mappedObject, this, [=](QObject* object) {
...
template<class T>
static inline void
_updateDockWidget(const char* name, bool enabled, bool show, Qt::DockWidgetArea pos, T callback)
{
...
bool MainWindow::setupStageView()
{
    // kiem tra stageview duoc an chua
    if (d->hiddenDockWindows.find("StageView") == std::string::npos) {
        // kiem tra stageview duoc hien thi chua
        if (findChild<QDockWidget*>(QLatin1String("StageView")) == nullptr) {
            // lay nhom tham so tu cau hinh nguoi dung
            // StageView
            auto group = App::GetApplication()
                             .GetUserParameter()
                             .GetGroup("BaseApp")
            ...
            // tao widget cho stageview
            auto* stageView = new Gui::DockWnd::StageView(this);

            // kiem tra chieu rong cua dockwidget
            bool restore = group->GetBool("RestoreWidth", true);  // mac dinh true
            stageView->setMinimumWidth(210);                      // dat chieu rong toi thieu

            stageView->setObjectName(
                QString::fromLatin1(QT_TRANSLATE_NOOP("QDockWidget", "Stage")));
            stageView->setWindowTitle(tr("Stage"));

            // dang ki DockWidget
            DockWindowManager* pDockMgr = DockWindowManager::instance();
            pDockMgr->registerDockWindow("StageView", stageView);

            addDockWidget(Qt::LeftDockWidgetArea, stageView);
            tabifyDockWidget(findChild<QDockWidget*>(QLatin1String("Std_ComboView")), stageView);

            // stageView->updateStageActions();
...
bool MainWindow::updateTreeView(bool show)
{
    if (d->hiddenDockWindows.find("Std_TreeView") == std::string::npos) {
        ...
        _updateDockWidget("Std_TreeView",
                          enabled,
                          show,
                          ...
                          [](QWidget* widget) {
                              ...
                              tree->setObjectName(
                                  QStringLiteral(QT_TRANSLATE_NOOP("QDockWidget", "Tree view")));
...
bool MainWindow::updatePropertyView(bool show)
{
    ...
    if (d->hiddenDockWindows.find("Std_PropertyView") == std::string::npos) {
        ...
        bool enabled = group->GetBool("Enabled", false);
        _updateDockWidget("Std_PropertyView",
                          enabled,
                          show,
                          Qt::RightDockWidgetArea,
                          [](QWidget* widget) {
                              ...
                              pcPropView->setObjectName(QStringLiteral(
...
bool MainWindow::updateTaskView(bool show)
{
    ...
    if (d->hiddenDockWindows.find("Std_TaskWatcher") == std::string::npos) {
        ...
        _updateDockWidget("Std_TaskWatcher",
                          enabled,
                          show,
                          Qt::RightDockWidgetArea,
                          [](QWidget* widget) {
                              ...
                              widget->setObjectName(
                                  QStringLiteral(QT_TRANSLATE_NOOP("QDockWidget", "Task List")));
                              return widget;
                          });
...
bool MainWindow::updateComboView(bool show)
{
    ...
    if (d->hiddenDockWindows.find("Std_ComboView") == std::string::npos) {
        ...
        _updateDockWidget("Std_ComboView",
                          enable,
                          show,
                          ...
                          [](QWidget* widget) {
                              ...
                              pcComboView->setObjectName(
                                  QStringLiteral(QT_TRANSLATE_NOOP("QDockWidget", "Model")));
        ...
        _updateDockWidget("Std_DAGView",
                          ...
                          [](QWidget* widget) {
                              ...
                              dagDockWindow->setObjectName(
                                  QStringLiteral(QT_TRANSLATE_NOOP("QDockWidget", "DAG View")));
...
void MainWindow::onWindowActivated(QMdiSubWindow* mdi)
{
    ...
    if (saveWB) {
        ...
        if (!currWb.isEmpty()) {
            ...
            mdi->setProperty(
                "ownWB",
                QString::fromStdString(WorkbenchManager::instance()->active()->name()));
        }
    }
    ...
    // But just testing whether the view is active and ignore it if not leads to other more serious
    // problems - at least under Linux. It seems to be a problem with the window manager. Under
    // Windows it seems to work though it's not really sure that it works reliably. Result: So, we
    // accept the first problem to be sure to avoid the second one.
...
void MainWindow::delayedStartup()
{
    ...
    if (SafeMode::SafeModeEnabled()) {
        auto safeModePopup = QMessageBox(QMessageBox::Information,
...
{
    auto recent = this->findChild<RecentFilesAction*>(QString::fromLatin1("recentFiles"));
    if (recent) {
...
{
    auto recent = this->findChild<RecentMacrosAction*>(QString::fromLatin1("recentMacros"));
    if (recent) {
        recent->appendFile(filename);
    }
}
...
void MainWindow::updateActions(bool delay)
{
    ...
    if (!d->activityTimer->isActive()) {
        ...
        if (d->activityTimer->thread() != QThread::currentThread()) {
            QMetaObject::invokeMethod(d->activityTimer,
                                      "start",
                                      Qt::QueuedConnection,
                                      Q_ARG(int, 150));
        }
...
void MainWindowP::restoreWindowState(const QByteArray& windowState)
{
    ...
    if (Base::Console().IsMsgTypeEnabled("ReportOutput", Base::ConsoleSingleton::MsgType_Wrn)) {
        Base::Console().SetEnabledMsgType("ReportOutput",
...
bool MainWindow::canInsertFromMimeData(const QMimeData* source) const
{
    ...
    return source->hasUrls() || source->hasFormat(_MimeDocObj) || source->hasFormat(_MimeDocObjX)
...
void MainWindow::loadUrls(App::Document* doc, const QList<QUrl>& urls)
{
    ...
    for (const auto& it : urls) {
        ...
        if (info.exists() && info.isFile()) {
            ...
            std::vector<std::string> module =
                App::GetApplication().getImportModules(info.completeSuffix().toLatin1());
            if (module.empty()) {
    ...
    QByteArray docName =
        doc ? QByteArray(doc->getName()) : qApp->translate("StdCmdNew", "Unnamed").toUtf8();
...
void StatusBarObserver::OnChange(Base::Subject<const char*>& rCaller, const char* sReason)
{
    ...
    if (strcmp(sReason, "colorText") == 0) {
        ...
        unsigned long col = rclGrp.GetUnsigned(sReason);
        this->critical =
            format.arg(QColor((col >> 24) & 0xff, (col >> 16) & 0xff, (col >> 8) & 0xff).name());
    }
}
