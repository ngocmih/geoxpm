...
// std::string activeStage = "Structures";  // Mặc định ban đầu là Geometry
...
/** \defgroup workbench Workbench Framework
 ...
 items in the toolbox, menus or the context menu and dockable windows that are shown to the user.
    The idea behind this concept is that the user should see only the functions that are required
 for the task that they are doing at this moment and not to show dozens of unneeded functions which
 the user never uses.
...
void Workbench::setupCustomToolbars(ToolBarItem* root,
                                    const Base::Reference<ParameterGrp> hGrp) const
{
...
void Workbench::createLinkMenu(MenuItem* item)
{
    ...
    *linkMenu << "Std_LinkMakeGroup" << "Std_LinkMake";
    ...
    const char* cmds[] = {"Std_LinkMakeRelative",
                          nullptr,
                          "Std_LinkUnlink",
                          "Std_LinkReplace",
                          "Std_LinkImport",
...
{
    auto it = std::find_if(staticMenuItems.begin(),
                           staticMenuItems.end(),
                           [cmd](const std::pair<std::string, std::string>& pmi) {
...
// --------------------------------------------------------------------
    ...
    qApp->translate("CommandGroup", "Tools");         //NATAES Ngoc - 1.2025
    ...
    // qApp->translate("CommandGroup", "Macro");        //NATAES Ngoc -2.2025
    ...
    qApp->translate("Workbench", "&Tools");           //NATAES Ngoc - 1.2025
    // qApp->translate("Workbench", "&Macro");           //NATAES Ngoc -2.2025
...
StdWorkbench::StdWorkbench()
    : Workbench()
{
    setupToolBars();  // Bắt buộc chạy trước để khởi tạo ToolBarItem*
}
...
MenuItem* StdWorkbench::setupMenuBar() const
{
    ...
    *file /*<< "Std_New" << "Std_Open"*/
        << "Std_RecentFiles" << "Separator" << "Std_CloseActiveWindow"
        << "Std_CloseAllWindows" << "Separator" /* << "Std_Save" << "Std_SaveAs"
         << "Std_SaveCopy" << "Std_SaveAll"*/
        << "Std_Revert" << "Separator" << "Std_Import"
        << "Std_MergeProjects"
        << "Std_ProjectInfo"
        //   << "Separator" << "Std_Print" << "Std_PrintPreview" << "Std_PrintPdf" << "Std_Export"
        //   //NATAES NGOC - 1.2025
        << "Separator"
        << "Std_ExecutionParameters" << "Std_Quit";
    ...
    *wnd << "Std_ActivateNextWindow" << "Std_ActivatePrevWindow" << "Separator"
    ...
    // Help                                       //NATAES Ngoc - 1.2025
    auto help = new MenuItem(menuBar);
    help->setCommand("&Help");
    *help << "Std_OnlineHelp" << "Std_WhatsThis"
          << "Separator"
          // Start page and additional separator are dynamically inserted here
          //   << "Std_FreeCADUserHub" << "Std_FreeCADForum" << "Std_FreeCADFAQ" << "Std_ReportBug"
          //   << "Separator"
          //   << "Std_RestartInSafeMode" << "Separator"
          //   << "Std_FreeCADPowerUserHub" << "Std_PythonHelp" << "Separator"
          //   << "Std_FreeCADWebsite" << "Std_FreeCADDonation"
          << "Std_About";

    // GeoXPM
    auto geoxpm = new MenuItem(menuBar);
    geoxpm->setCommand("&GeoXPM");
    *geoxpm << "Std_FillBox" << "Std_SoilTests" << "Std_Import3Dobject"
            << "Std_Materialproperties"
            << "Std_SoilStructureInteraction"
            << "Std_SoilWaterCouplingParametersInput"
            << "Std_AutofitSimulationDomain" << "Std_MeshingOrder"
            << "Std_RemeshSSI"
            << "Std_ExecuteSimulations"
            << "Std_ExecutionParameters" << "Std_ExportOutputToTabFormat"
            << "Std_ExportOutputToParaview" << "Std_OpenParaView"
            << "Std_GenerateParticles";

    return menuBar;
}

// NATAES Ngoc - 1.2025 - start
