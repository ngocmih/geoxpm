...
void ThemeSelectorWidget::setupUi()
{
    ...
    auto* buttonLayout = gsl::owner<QVBoxLayout*>(new QVBoxLayout);
...
void ThemeSelectorWidget::preselectThemeFromSystemSettings()
{
    ...
    if (styleSheetName == QString::fromStdString(nullStyle)) {
        ...
        // themeChanged(theme);
        themeChanged(Theme::Classic);  //NataES
    }
}
...
void ThemeSelectorWidget::retranslateUi()
{
    ...
    _buttons[static_cast<int>(Theme::Classic)]->setText(tr("GeoXPM Classic", "Visual theme name"));
}
